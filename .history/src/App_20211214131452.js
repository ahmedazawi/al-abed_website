...
function App() {
  ...
  const fetchData = () => {
    axios
      ...
      .then((response) => {
        console.log(response.data[0].data.Demand);
        setExchange({ Supply: response.data[0].data.Supply });
        setExchange({ Demand: response.data[0].data.Demand });
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    <div className="App">
      <ContextApi.Provider
        value={{ lang, supply= exchange.Supply, demand:=exchange.Demand }}
      >
        <BrowserRouter>
